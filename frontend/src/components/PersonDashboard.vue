  ...
  <div class="dashboard">
    <div class="header">
      <div>
        <h1>{{ fullName }}</h1>
        <div class="meta-tags">
          <span v-if="person.birth_date" class="meta-tag">{{ formatBirthDate(person.birth_date) }}</span>
    ...
    <div class="card results-card">
      ...
      <div v-else-if="results.length" class="results-list">
        <a v-for="r in results" :key="r.id" :href="r.url" target="_blank" class="result-item">
...
const fullName = computed(() => [props.person.last_name, props.person.first_name, props.person.middle_name].filter(Boolean).join(' '))

function typeLabel(t) { return { news: 'новости', image: 'фото', social: 'соцсети', registry: 'реестр' }[t] || t }
function formatBirthDate(d) { return d ? new Date(d).toLocaleDateString('ru-RU', { day: 'numeric', month: 'long', year: 'numeric' }) : '' }
function formatDate(d) { return d ? new Date(d).toLocaleDateString('ru-RU', { day: 'numeric', month: 'short' }) : '' }
function barWidth(count, sources) { const max = Math.max(...Object.values(sources)); return max ? Math.round((count / max) * 100) : 0 }

async function load() {
  loading.value = true
  try {
    const [s, r] = await Promise.all([api.getPersonStats(props.person.id), api.getPersonResults(props.person.id, activeFilter.value)])
    stats.value = s; results.value = r
  } finally { loading.value = false }
}

async function setFilter(val) {
  activeFilter.value = val; loading.value = true
  try { results.value = await api.getPersonResults(props.person.id, val) }
  finally { loading.value = false }
}
...
async function triggerSearch() {
  ...
  try {
    ...
    const poll = setInterval(async () => {
      ...
      if (t.status === 'SUCCESS' || t.status === 'FAILURE') { clearInterval(poll); searching.value = false; await load() }
    }, 2000)
...
.meta-tag { background: #1e2235; border: 1px solid #2d3148; border-radius: 20px; padding: 2px 10px; font-size: 11px; color: #94a3b8; }
.header-actions { display: flex; gap: 8px; flex-shrink: 0; }
.profile-card { display: flex; align-items: flex-start; gap: 24px; flex-wrap: wrap; }
.profile-photo { width: 80px; height: 80px; border-radius: 12px; overflow: hidden; flex-shrink: 0; background: #0f1117; border: 1px solid #2d3148; display: flex; align-items: center; justify-content: center; }
...
.result-item { display: flex; align-items: flex-start; gap: 14px; padding: 14px 0; border-bottom: 1px solid #1e2235; text-decoration: none; transition: background 0.1s; cursor: pointer; }
.result-item:last-child { border-bottom: none; }
.result-item:hover { background: #1e2235; margin: 0 -20px; padding: 14px 20px; border-radius: 8px; }
.result-thumb { width: 48px; height: 48px; border-radius: 8px; object-fit: cover; flex-shrink: 0; border: 1px solid #2d3148; }
.result-thumb-placeholder { width: 48px; height: 48px; border-radius: 8px; flex-shrink: 0; background: #1e2235; border: 1px solid #2d3148; display: flex; align-items: center; justify-content: center; }
...
.result-snippet { font-size: 12px; color: #64748b; line-height: 1.5; overflow: hidden; display: -webkit-box; -webkit-line-clamp: 2; -webkit-box-orient: vertical; }
.type-badge { display: inline-flex; align-items: center; padding: 1px 7px; border-radius: 20px; font-size: 10px; font-weight: 600; letter-spacing: 0.03em; }
