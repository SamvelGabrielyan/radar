...
from tasks.parser import analyze_sentiment, fetch_mentions, build_sources, USER_AGENTS, _random_ua, _extract_published_at
...
def test_build_sources_includes_bing():
    """Новый тест: Bing News должен быть в списке источников."""
    sources = build_sources("Test")
    names = [s["name"] for s in sources]
    assert "Bing News" in names

def test_build_sources_includes_google_ru_and_en():
    sources = build_sources("Test")
    names = [s["name"] for s in sources]
    assert "Google News RU" in names
    assert "Google News EN" in names
...
def test_random_ua_from_pool():
    for _ in range(20):
        assert _random_ua() in USER_AGENTS

def test_ua_pool_has_variety():
    assert len(USER_AGENTS) >= 3


# ─── _extract_published_at ────────────────────────────────────────────────────

def test_extract_published_at_with_published():
    entry = MagicMock()
    entry.published_parsed = (2025, 6, 15, 10, 30, 0, 0, 0, 0)
...
def test_extract_published_at_fallback_updated():
    entry = MagicMock()
    entry.published_parsed = None
    entry.updated_parsed = (2025, 1, 1, 0, 0, 0, 0, 0, 0)
    entry.created_parsed = None
    result = _extract_published_at(entry)
    assert result is not None

def test_extract_published_at_none():
    entry = MagicMock()
    entry.published_parsed = None
    entry.updated_parsed = None
    entry.created_parsed = None
...
def test_fetch_mentions_skips_empty(mock_http, mock_feedparser):
    mock_http.return_value = "<rss/>"
    mock_feedparser.return_value = make_mock_feed([
        make_entry("", "https://news.com/1"),
        make_entry("Title", ""),
        make_entry("Good title", "https://news.com/good"),
    ])
    results = fetch_mentions(["Apple"])
    for r in results:
        assert r["title"] != ""
